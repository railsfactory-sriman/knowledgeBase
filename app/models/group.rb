class Group
  ...
  include Railsknowledgebase::Models::CustomHtmlMethods
  ...
  def self.find_file_from_params(params, request)
    if request.path =~ /\/(logo|big|medium|small|css|favicon)\/([^\/\.?]+)/
      ...
      logo = @group.has_logo? ? @group.logo : Railsknowledgebase::FileWrapper.new("#{Rails.root}/public/images/logo.png", "image/png")
      case $1
      when "logo"
        logo
      when "big"
