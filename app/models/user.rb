...
class User
  ...
  include Railsknowledgebase::Models::Networks
...
Time.zone.now ? 1 : 0)
  ...
  def self.find_file_from_params(params, request)
    if request.path =~ %r{/(avatar|big|medium|small)/([^/\.\?]+)}
      @user = User.find($2)
      avatar = @user.has_avatar? ? @user.avatar : Railsknowledgebase::FileWrapper.new("#{Rails.root}/public/images/avatar-25.png", "image/png")
      case $1
      when "avatar"
        avatar
      when "big"
